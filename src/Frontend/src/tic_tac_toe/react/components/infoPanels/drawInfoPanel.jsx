// src/Frontend/src/react/react/components/drawInfoPanel.jsx
...
export default function DrawInfoPanel({
  ...
  const title   = { ...styles?.subtitleStyle, margin: 0, fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 900, fontSize: 'clamp(28px, 4vw, 52px)', lineHeight: 1.15, textAlign: 'center', color: '#93C5FD' };
  ...
  const vsText  = { fontWeight: 900, fontSize: 'clamp(22px, 3.2vw, 42px)', lineHeight: 1.2, color: '#BFDBFE' };
  ...
  const label = { fontWeight: 700, fontSize: 'clamp(14px, 2.0vw, 20px)', textAlign: 'right', justifySelf: 'end', color: '#C7D2FE' };
  ...
  return (
    <Box style={card}>
      <div ref={contentRef} style={contentWrap}>
        ...
        <div style={subtitle}>Vyrovnaná bitva — remíza.</div>
