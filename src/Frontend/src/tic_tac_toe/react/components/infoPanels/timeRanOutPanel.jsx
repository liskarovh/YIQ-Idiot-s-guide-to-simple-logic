// src/Frontend/src/react/tic_tac_toe/components/timeRanOutPanel.jsx
import React, { useLayoutEffect, useMemo, useRef, useState } from 'react';
import Box from '../../../../components/Box';
import styles from '../../../../Styles';
import PlayerBadge from '../playerBadge.jsx';

export default function TimeRanOutPanel({
  ...
  // Reuse the same card look as LoseInfoPanel
  const card = {
  ...
  const title    = { ...styles?.subtitleStyle, margin: 0, fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 900, fontSize: 'clamp(28px, 4vw, 52px)', lineHeight: 1.15, textAlign: 'center', color: '#FF6B6B' };
  ...
  const vsText   = { fontWeight: 900, fontSize: 'clamp(22px, 3.2vw, 42px)', lineHeight: 1.2, color: '#FFD1D1' };
  ...
  const label = { fontWeight: 700, fontSize: 'clamp(14px, 2.0vw, 20px)', textAlign: 'right', justifySelf: 'end', color: '#FFC2C2' };
  ...
  return (
    <Box style={card}>
      <div ref={contentRef} style={contentWrap}>
        <h3 style={title}>Time ran out</h3>
        <div style={subtitle}>Your turn timer expired.</div>
