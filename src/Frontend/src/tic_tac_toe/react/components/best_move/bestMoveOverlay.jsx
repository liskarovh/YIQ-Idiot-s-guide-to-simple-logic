...
export default function BestMoveOverlay({
  anchorRef,       // e.g., statsRef (wrapper around the Game info card)
  open,
  onClose,
  move,
  explain,         // plain string fallback
  ...
  return createPortal(
    <div
      ...
      style={{
        ...
        zIndex: 2000, borderRadius: radius,
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
          ...
          {typeof rollouts === "number" && (
            <span style={{
              marginLeft: 8, padding: "2px 8px", borderRadius: 9999, fontSize: 12, fontWeight: 600,
              background: "rgba(148,163,184,0.08)", border: "1px solid rgba(148,163,184,0.35)", color: "#cbd5e1"
            }}>
              rollouts: {rollouts}
            </span>
          )}
          ...
          <button
            onClick={onClose}
            aria-label="Close"
            style={{ marginLeft: "auto", background: "transparent", border: 0, color: "#CBD5E1", fontSize: 22, cursor: "pointer", lineHeight: 1 }}
          >
            ×
          </button>
        </div>
        ...
        {Array.isArray(move) && (
          <div style={{
            ...
            color: "#e2e8f0", padding: "8px 10px", borderRadius: 10, margin: "6px 0 8px"
          }}>
        ...
        {reasons.length > 0 && (
          <>
            <div style={{ color: "#94a3b8", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 6 }}>
              Why this move
            </div>
        ...
        {/* Optional: potential winningSequence preview */}
        {winSeq.length > 0 && (
